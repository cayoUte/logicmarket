...
def SearchInput(parent, placeholder="Buscar...", width=400, height=48, radius=24, bg_color="#EEF2F6", text_color="#1C1B1F"):
    """
    Input de búsqueda estilo MD3.
    """
    try:
    ...
    def draw():
        ...
        if cache_key not in state["image_cache"]:
            ...
            draw_ctx.rounded_rectangle((0, 0, W-1, H-1), radius=R, fill=state["current_bg"])
        ...
        # Icono Lupa (SVG)
        tk_icon = utils.load_svg_icon("gui/assets/icons/search.svg", 20, "#444746", state["current_bg"])
        canvas.create_image(24, height/2, image=tk_icon, anchor="center")
        canvas.icon_ref = tk_icon

    draw()
    entry = tk.Entry(canvas, bg=bg_color, bd=0, fg=text_color, font=get_font("body"), highlightthickness=0, insertbackground=text_color)
    canvas.create_window(48, height/2, window=entry, width=width-70, height=height-14, anchor="w")
