...
def TextField(
    ...
    state = {"focused": False, "error": False, "hover": False}

    wrapper = tk.Frame(parent, bg=bg_parent, highlightthickness=0)

    container = tk.Frame(
        wrapper, bg=COLORS["bg_idle"], width=width, height=56, cursor="xterm"
    )
    ...
    if leading_icon:
        icon_lead = IconButton(
            container,
            icon_path=leading_icon,
            on_click=lambda: entry.focus_set(),
            size=24,
            bg_parent=COLORS["bg_idle"],
            variant="neutral",
            ui_mode=ui_mode,
        )
    ...
    entry.place(
        x=padding_left, y=24, width=width - padding_left - padding_right, height=24
    )
    ...
    def update_visuals(force_idle=False):
        ...
        entry.configure(
            bg=bg_current, fg=COLORS["text"], insertbackground=COLORS["caret"]
        )
