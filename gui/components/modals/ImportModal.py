...
def ImportModal(parent, product_data, on_confirm, ui_mode="light"):
    ...
    tk.Label(main, text="Importar al Inventario", font=get_font("h4"), fg=title_color, bg=bg_color, anchor="w").pack(fill="x", pady=(0, 16))

    # --- TARJETA SOLO LECTURA (Datos API) ---
    card = tk.Frame(main, bg=card_bg, padx=16, pady=16)
    card.pack(fill="x", pady=(0, 24))

    tk.Label(card, text=product_data.get('name', 'Sin Nombre'), font=("Segoe UI", 11, "bold"), 
             fg=title_color, bg=card_bg, wraplength=320, anchor="w", justify="left").pack(fill="x")
    
    meta = f"{product_data.get('brand', 'N/A')} • {product_data.get('code', 'N/A')}"
    tk.Label(card, text=meta, font=("Segoe UI", 9), fg=body_color, bg=card_bg, anchor="w").pack(fill="x", pady=(4, 0))

    # --- FORMULARIO INVENTARIO ---
    inp_price = TextField(main, label="Precio Venta ($) *", width=350, ui_mode=ui_mode, bg_parent=bg_color)
    inp_price.pack(fill="x", pady=(0, 12))

    inp_stock = TextField(main, label="Stock Inicial *", width=350, ui_mode=ui_mode, bg_parent=bg_color)
    ...
    def handle_save():
        try:
            ...
            if p <= 0 or s < 0 or not sup: raise ValueError
    ...
    Pillow_Button(actions, text="IMPORTAR", on_click=handle_save, variant="primary", bg_parent=bg_color, ui_mode=ui_mode, dimensions=(110,40,20)).pack(side="right")
