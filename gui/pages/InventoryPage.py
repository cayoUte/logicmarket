...
class InventoryTable(tk.Frame):
    def __init__(self, parent, colors, on_action_click):
        ...
        self.colors = colors
        self.on_action_click = on_action_click # Callback (product, x, y)
        
        self.bg = colors["background"]
        self.text_primary = colors["text"]
        
        # Lógica de colores secundarios
        ...
        # --- CONFIGURACIÓN DE COLUMNAS (Grid Layout) ---
        # weight=1 significa que se estira, minsize evita solapamiento
        self.col_config = {
            0: {"weight": 1, "minsize": 250}, # Producto (Nombre + Avatar)
            1: {"weight": 0, "minsize": 150}, # Categoría
            2: {"weight": 0, "minsize": 100}, # Stock
            3: {"weight": 0, "minsize": 100}, # Precio
            4: {"weight": 0, "minsize": 60},  # Acción
        }
        ...
        for col, cfg in self.col_config.items():
            self.header.grid_columnconfigure(col, weight=cfg["weight"], minsize=cfg["minsize"])
        ...
        # Window para el frame interno
        self.canvas_window = self.canvas.create_window((0, 0), window=self.scrollable_frame, anchor="nw")
        
        self.canvas.configure(yscrollcommand=self.scrollbar.set)
        
        # Ajustar ancho al redimensionar ventana
    ...
    def create_row(self, prod, is_last, ui_mode):
        ...
        # Usamos .get() para soportar datos antiguos o parciales
        ...
        # Configurar columnas de la fila (Igual que header)
        for col, cfg in self.col_config.items():
            row.grid_columnconfigure(col, weight=cfg["weight"], minsize=cfg["minsize"])

        # --- COL 0: INFO (Avatar + Textos) ---
        ...
        try:
            ...
            if img:
                ...
                tk.Label(cell_info, text="📦", bg=self.bg, font=("Segoe UI Emoji", 14)).pack(side="left", padx=(0, 10))
        except:
        ...
        # Nombre (Negrita)
        tk.Label(txt_frame, text=str(name)[:35], bg=self.bg, fg=self.text_primary, 
                 font=("Segoe UI", 10, "bold"), anchor="w").pack(fill="x")
        # Subtítulo (Código • Marca)
        tk.Label(txt_frame, text=f"{code} • {brand}", bg=self.bg, fg=self.text_secondary, 
                 font=("Segoe UI", 9), anchor="w").pack(fill="x")

        # --- COL 1: CATEGORÍA ---
        tk.Label(row, text=str(category), bg=self.bg, fg=self.text_secondary, anchor="w").grid(row=0, column=1, sticky="ew", padx=5)

        # --- COL 2: STOCK ---
        try:
            s_val = int(stock)
            # Color rojo si stock bajo (< 5)
            s_color = get_app_color("error", "Basic") if s_val < 5 else self.text_primary
        except:
            s_val, s_color = 0, self.text_primary
        
        tk.Label(row, text=f"{s_val} u.", bg=self.bg, fg=s_color, 
                 font=("Segoe UI", 10, "bold"), anchor="w").grid(row=0, column=2, sticky="ew", padx=5)

        # --- COL 3: PRECIO ---
        try: p_txt = f"${float(price):.2f}"
        except: p_txt = "$0.00"
        tk.Label(row, text=p_txt, bg=self.bg, fg=self.text_primary, anchor="w").grid(row=0, column=3, sticky="ew", padx=5)
        ...
        btn = IconButton(
            ...
            variant="neutral", 
            ui_mode=ui_mode, 
            on_click=None # Callback manual abajo
        )
        
        # Captura manual de coordenadas para el menú contextual
        def manual_click(e):
        ...
        def on_enter(e): set_hover_bg(row, self.hover_color)
        def on_leave(e): set_hover_bg(row, self.bg)
...
def InventoryPage(parent, store_funcs):
    ...
    # Botón Circular "Nuevo Producto"
    btn_add = IconButton(
        header_frame,
        icon_path="add_circle.svg", # Debe existir en assets/icons/
        size=32,
        variant="primary",
        bg_parent=bg_color,
        ui_mode=current_mode,
    ...
    def update_ui(new_state):
        ...
        if query:
            filtered = []
            for p in full_data:
                # Concatenamos campos clave para buscar
                p_str = (str(p.get("name")) + str(p.get("code")) + str(p.get("brand"))).lower()
                if query in p_str:
                    filtered.append(p)
        else:
            filtered = full_data
